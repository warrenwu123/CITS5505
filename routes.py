...
def leaderboard():
    ...
    # Most Active Users: 按总锻炼时长排序
    most_active = sorted(users, key=lambda u: u.total_duration, reverse=True)
    # Achievement Leaders: 按成就数量排序
    achievement_leaders = sorted(users, key=lambda u: len(u.user_achievements), reverse=True)
